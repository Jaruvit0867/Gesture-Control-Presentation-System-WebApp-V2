...
const gestureInfo = {
    ...
    STABILIZING: { label: 'Detection Paused', color: 'text-rose-400', icon: '✊' },
};
...
export function GestureCamera({
    ...
    useEffect(() => {
        // If it's anything else, update immediately
        if (gesture.name !== 'SCANNING') {
            setDisplayGesture(gesture);
            return;
        }

        // If it's SCANNING, only update if we've been scanning for a while
        ...
        if (displayGesture.name !== 'SCANNING') {
            ...
            // Already showing SCANNING, just keep it updated with latest metrics
            setDisplayGesture(gesture);
        }
    ...
    return (
        <div className={`glass rounded-3xl p-5 relative overflow-hidden border border-white/5 shadow-2xl ${isMini ? 'p-2 opacity-80 hover:opacity-100 transition-all duration-500 scale-95 hover:scale-100 shadow-none' : ''}`}>
                                        ...
                                        <div>
                                            ...
                                            {!isMini && (
                                                <p className="text-[10px] text-slate-500 font-bold uppercase tracking-tighter mt-0.5">
                                                    {displayGesture.name === 'READY' ? 'System Active' :
                                                        displayGesture.name === 'PAUSED' ? 'System Paused' :
                                                            'Waiting for Gesture'}
                                                </p>
                                            )}
                                        </div>
                                            ...
                                            <div className="relative w-12 h-12">
                                                <svg className="w-12 h-12 transform -rotate-90">
                                                    ...
                                                    <motion.circle
                                                        ...
                                                        animate={{ strokeDashoffset: 113.1 - (113.1 * displayGesture.confidence) }}
